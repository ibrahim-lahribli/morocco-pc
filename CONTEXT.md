...
- **Engine (pure JS, `src/recommendation/`)** — implemented and covered by unit tests (`npm run test:unit`; 636 tests as of 2026-09-21):
...
Currently next:
- (1) Decisions 17–19 RESOLVED (2026-09-21) — orchestrator contract, ranking, persistence (recorded; query/ loader now implemented, orchestrator pending).
- (2) DONE 2026-09-21 — Query data-loading layer (Decision 10 / 17.1) — `loadQueryInput` in `src/recommendation/query/` (Rule 1 constant, fail-closed `use_case`, strict NUMERIC-string budget); 636 unit tests pass.
- (3) NEXT — Orchestrator, no writes — wiring query/ → 2C → Stage 1 → 2D → retention → 3 → 4 inside one REPEATABLE READ READ ONLY snapshot transaction (`runRecommendationSnapshot` owns BEGIN/end; ROLLBACK on ANY thrown error — a malformed query id surfaces as Postgres 22P02, which aborts the snapshot transaction).
