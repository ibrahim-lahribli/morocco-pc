...
- **Engine (pure JS, `src/recommendation/`)** — implemented and covered by unit tests (`npm run test:unit`; 788 tests as of 2026-09-24):
...
- (3b) Decision 20 — Post-ranking (CPU, GPU) pair diversity selection (RESOLVED 2026-09-22, implemented; reconciled with real measured numbers 2026-09-23).
...
NOT implemented:
- Engine 6 — explanation generation: absent; `explanation` is still hard-coded `null` everywhere. (The end-to-end query → build → score → rank → diversity-select → persist pipeline is implemented via `orchestrator/full-run.js`, Decision 21, RESOLVED 2026-09-23; Engine 5a ranking, Decision 20 diversity selection, and Engine 5b persistence are all wired in.)
